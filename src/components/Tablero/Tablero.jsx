...
const Tablero = ({ onJuegoTerminado }) => {
  ...
  const handleComenzar = async () => {
    ...
    await barajar(tableroLogic);
    await new Promise(resolve => setTimeout(resolve, 1000));
    await repartir({ ...tableroLogic, posicionesCartas });
  };
            ...
            >
              <div className="space-y-8">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {/* Solo mostrar el botón Comenzar al inicio */}
                  {!juegoIniciado && (
                    <button
                      onClick={handleComenzar}
                      disabled={tableroLogic.estaBarajando || tableroLogic.estaRepartiendo || tableroLogic.estaJugando || tableroLogic.modoManual}
                      className={`px-6 py-2.5 rounded-md font-bold tracking-wide transition-all duration-200 relative overflow-hidden group bg-gradient-to-r from-purple-700 to-purple-500 border-2 border-purple-400 text-white shadow-lg hover:from-purple-800 hover:to-purple-600 disabled:bg-gray-900 disabled:border-gray-700 disabled:text-gray-500 disabled:shadow-none`}
                    >
                      {tableroLogic.estaBarajando || tableroLogic.estaRepartiendo ? (
                  ...
                  {/* Solo mostrar los botones de juego automático y manual después de comenzar, y NO duplicar */}
                  {juegoIniciado && !showGamePanel && (
                    <>
                      <button
                  ...
                  {/* (Eliminados los botones de juego automático y manual fuera del bloque condicional) */}
                </div>
              </div>
            </motion.div>
          )}
